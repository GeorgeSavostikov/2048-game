...
function searchWinnerCell() {
    if (winner.length !== 0) messageWin.classList.remove("hidden");
}
...
function slide(row) {
    ...
    for(let i = 0; i < currentRow.length - 1; i++)if (currentRow[i] === currentRow[i + 1]) {
        currentRow[i] *= 2;
        currentRow[i + 1] = 0;
        score += currentRow[i];
    }
...
function canTilesSlide() {
    ...
    for(let r = 0; r < rows; r++)for(let c = 0; c < columns; c++){
        ...
        if (c > 0 && tile === board[r][c - 1]) return true;
        if (c < columns - 1 && tile === board[r][c + 1]) return true;
        if (r > 0 && tile === board[r - 1][c]) return true;
        if (r < rows - 1 && tile === board[r + 1][c]) return true;
    }
    return false;
}
function gameOver() {
    if (!canTilesSlide()) messageLose.classList.remove("hidden");
}
function arraysEqual(originalRow, row) {
    if (originalRow.length !== row.length) return false;
    for(let i = 0; i < originalRow.length; i++){
        if (originalRow[i] !== row[i]) return false;
    }
    return true;
}
function slideLeft() {
    for(let r = 0; r < rows; r++){
        ...
        if (!arraysEqual(originalRow, row)) tilesMoved = true;
    }
}
function slideRight() {
    for(let r = 0; r < rows; r++){
